...
ConstantBuffer<SceneParameters> SceneParams : register(b2); // Scene params
...
ConstantBuffer<ModelParameters> ModelParams : register(b3); // Model params
...
StructuredBuffer<Triangle> Triangles : register(t4); // Index buffer
StructuredBuffer<float3>   Positions : register(t5); // Position buffer
StructuredBuffer<float3>   Normals   : register(t6); // Normal buffer
...
void MyClosestHitShader(inout RayPayload payload, in MyAttributes attr)
{
    ...
    // Material variables
    float3 albedo    = MaterialParams[instIdx].albedo;
    float  roughness = MaterialParams[instIdx].roughness;
    float  metalness = MaterialParams[instIdx].metalness;
    float3 F0        = MaterialParams[instIdx].F0;
