...
const char* gShaderShadowMISS = R"(
#version 460
#extension GL_EXT_ray_tracing : enable
...
void main()
{
    ...
    // Diffuse
    vec3 lightPos = cam.lightPosition;
    ...
    if (payload.recursionDepth < 5) {
        ...
        traceRayEXT(
            ...
            direction,            // direction
            tmax,                 // Tmax
            1);                   // payload -> layout(location = 1) rayPayloadInEXT ShadowPayload shadowPayload;
    ...
    if (gl_PrimitiveID > 0) {
        ...
        if (payload.recursionDepth < 5) {
            ...
            traceRayEXT(
                ...
                direction,            // direction
                tmax,                 // Tmax
                0);                   // payload -> layout(location = 0) rayPayloadInEXT RayPayload payload;
        ...
        vec3 color = ((kD * d + kS * s) * SPHERE) + reflectedColor * (1 - 0.2 * shadow);
        payload.hitValue = color;     
    }
    else {
        payload.hitValue = d * GROUND * (1 - 0.4 * shadow);
    }
}
)";
...
void CreateShaderModules(
    ...
    const std::vector<uint32_t>& spirvShadowMISS,
    ...
    VkShaderModule*              pModuleShadowMISS,
...
void CreateRayTracingPipeline(
    ...
    VkShaderModule   moduleShadowMISS,
...
int main(int argc, char** argv)
{
    ...
    std::vector<uint32_t> spirvShadowMISS;
    ...
    std::vector<uint32_t> spirvRINT;
    {
        ...
        if (res != COMPILE_SUCCESS) {
            std::stringstream ss;
            ss << "\n"
               << "Shader compiler error (shadow MISS): " << errorMsg << "\n";
            GREX_LOG_ERROR(ss.str().c_str());
            return EXIT_FAILURE;
        }
    ...
    VkShaderModule moduleShadowMISS = VK_NULL_HANDLE;
    ...
    // Window
    // *************************************************************************
    auto window = GrexWindow::Create(gWindowWidth, gWindowHeight, "006_basic_shadow_dynamic_vulkan");
    if (!window) {
    ...
    while (window->PollEvents()) {
        ...
        // Update light position
        {
            ...
            // Hard code offset for now
            //
            // 128 = 2 * sizeof(mat4);
            // 16  = aligned sizeof(vec3)
            //
            vec3* pLightPosition = reinterpret_cast<vec3*>(pUniformBufferAddr + 128 + 16);
            *pLightPosition      = vec3(r * cos(t), 25, r * sin(t));
        }
        ...
        CHECK_CALL(vkBeginCommandBuffer(cmdBuf.CommandBuffer, &vkbi));
        {
            ...
            const uint32_t alignedGroupHandleSize = Align(
            ...
            rgenShaderSBTEntry.stride                          = alignedGroupHandleSize;
            rgenShaderSBTEntry.size                            = alignedGroupHandleSize;

            VkStridedDeviceAddressRegionKHR missShaderSBTEntry = {};
            missShaderSBTEntry.deviceAddress                   = GetDeviceAddress(renderer.get(), &missSBT);
            missShaderSBTEntry.stride                          = alignedGroupHandleSize;
            missShaderSBTEntry.size                            = 2 * alignedGroupHandleSize;

            VkStridedDeviceAddressRegionKHR hitgShaderSBTEntry = {};
            hitgShaderSBTEntry.deviceAddress                   = GetDeviceAddress(renderer.get(), &hitgSBT);
            hitgShaderSBTEntry.stride                          = Align(alignedGroupHandleSize + 8, rayTracingProperties.shaderGroupHandleSize);
            hitgShaderSBTEntry.size                            = Align(alignedGroupHandleSize + 8, rayTracingProperties.shaderGroupHandleSize);
    ...
    vmaUnmapMemory(renderer->Allocator, uniformBuffer.Allocation);
    ...
    VkShaderModule*              pModuleRINT)
{
    ...
    {
        VkShaderModuleCreateInfo createInfo = {VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO};
        createInfo.codeSize                 = SizeInBytes(spirvShadowMISS);
        createInfo.pCode                    = DataPtr(spirvShadowMISS);

        CHECK_CALL(vkCreateShaderModule(pRenderer->Device, &createInfo, nullptr, pModuleShadowMISS));
    }
    ...
    VkPipeline*      pPipeline)
{
    ...
    // Shadow Miss
    {
        VkPipelineShaderStageCreateInfo createInfo = {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO};
        createInfo.stage                           = VK_SHADER_STAGE_MISS_BIT_KHR;
        createInfo.module                          = moduleShadowMISS;
        createInfo.pName                           = "main";
    ...
    // Closest hit + Intersection
    {
        ...
        createInfo.closestHitShader                     = 3; // shaderStages[3]
        createInfo.anyHitShader                         = VK_SHADER_UNUSED_KHR;
        createInfo.intersectionShader                   = 4; // shaderStages[4]
    ...
    VulkanBuffer*                                    pHitGroupSBT)
{
    ...
    char* pShaderGroupHandleShadowMISS = groupHandlesData.data() + 2 * groupHandleSize;
    char* pShaderGroupHandleHITG       = groupHandlesData.data() + 3 * groupHandleSize;
    ...
    {
        //
        // There are two shader records for the miss shader binding table.
        ...
        //  +---------------------+
        //  | MyMissShadowShader  | offset = 32
        //  +---------------------+
        //

        CHECK_CALL(CreateBuffer(
            pRenderer,                  // pRenderer
            2 * alignedGroupHandleSize, // srcSize
            nullptr,                    // pSrcData
            usageFlags,                 // usageFlags
            shaderGroupBaseAlignment,   // minAlignment
            pMissSBT));                 // pBuffer
        ...
        {
            char* pData = nullptr;
            CHECK_CALL(vmaMapMemory(pRenderer->Allocator, pMissSBT->Allocation, reinterpret_cast<void**>(&pData)));

            // gShaderMISS
            // 
            // Corresponds to traceRayEXT function call with missIndex=0 (lighting) in MyClosestHitShader
            //
            memcpy(pData, pShaderGroupHandleMISS, groupHandleSize);
            pData += alignedGroupHandleSize;

            // gShaderShadowMISS
            // 
            // Corresponds to traceRayEXT function call with missIndex=1 (shadow) in MyClosestHitShader
            //
            memcpy(pData, pShaderGroupHandleShadowMISS, groupHandleSize);

            vmaUnmapMemory(pRenderer->Allocator, pMissSBT->Allocation);
        }
    }
...
void CreateUniformBuffer(VulkanRenderer* pRenderer, VulkanBuffer* pBuffer)
{
    struct Camera
    {
        ...
        vec3      eyePosition;
        float     __pad0;
        vec3      lightPosition; // Updated in main loop
    };
