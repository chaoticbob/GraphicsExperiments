...
void MyRaygenShader()
{
    ...
    TraceRay(
        ...
        0,                     // MissShaderIndex (0 = lighting, 1 = shadow)
        ray,                   // Ray
        payload);              // Payload
...
[shader("miss")]
void MyMissShadowShader(inout ShadowPayload payload)
{
    payload.hit = false;
}
...
void MyClosestHitShader(inout RayPayload payload, in ProceduralPrimitiveAttributes attr)
{
    ...
    if (currentRecursionDepth < 5) {
        ...
        TraceRay(
            ...
            0,                     // RayContributionToHitGroupIndex
            1,                     // MultiplierForGeometryContributionToHitGroupIndex
            1,                     // MissShaderIndex (0 = lighting, 1 = shadow)
            ray,                   // Ray
...
static LPCWSTR gMissShadowShaderName   = L"MyMissShadowShader";
...
int main(int argc, char** argv)
{
    ...
    // Window
    // *************************************************************************
    auto window = GrexWindow::Create(gWindowWidth, gWindowHeight, "006_basic_shadow_dynamic_d3d12");
    if (!window) {
    ...
    // Main loop
    // *************************************************************************
    char* pConstantBufferAddr = nullptr;
    CHECK_CALL(constantBuffer->Map(0, nullptr, reinterpret_cast<void**>(&pConstantBufferAddr)));
    ...
    while (window->PollEvents()) {
        ...
        // Update light position
        {
            ...
            //
            vec3* pLightPosition = reinterpret_cast<vec3*>(pConstantBufferAddr + 128 + 16);
    ...
    ID3D12StateObject**  ppStateObject)
{
    ...
    D3D12_EXPORT_DESC missShadowExport = {};
    missShadowExport.Name              = gMissShadowShaderName;
    missShadowExport.ExportToRename    = nullptr;
    missShadowExport.Flags             = D3D12_EXPORT_FLAG_NONE;
    ...
    exports.push_back(missShadowExport);
    ...
    ID3D12Resource**   ppHitGroupSRT)
{
    ...
    void* pMissShadowShaderIdentifier = stateObjectProperties->GetShaderIdentifier(gMissShadowShaderName);
...
void CreateConstantBuffer(DxRenderer* pRenderer, ID3D12Resource* pRayGenSRT, ID3D12Resource** ppConstantBuffer)
{
    ...
    size_t offset = offsetof(Camera, lightPosition);
