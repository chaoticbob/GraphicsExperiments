...
void CreateBLASes(
    ...
    std::vector<VulkanAccelStruct>& outBLASes);
void CreateTLAS(
    VulkanRenderer*                       pRenderer,
    const std::vector<VulkanAccelStruct>& BLASes,
...
int main(int argc, char** argv)
{
    ...
    {
        auto source = LoadString("projects/023_raytracing_multi_instance/shaders.hlsl");
    ...
    // *************************************************************************
    std::vector<VulkanAccelStruct> BLASes;
    CreateBLASes(
        renderer.get(),
        geometries,
        BLASes);
    ...
    CreateTLAS(renderer.get(), BLASes, &TLAS);
...
void CreateBLASes(
    ...
    std::vector<VulkanAccelStruct>& outBLASes)
{
    outBLASes.resize(geometries.size());
    ...
    for (uint32_t i = 0; i < geometries.size(); ++i)
    {
        ...
        const uint32_t numTriangles = geometries[i].indexCount / 3;
        ...
        buildGeometryInfo.pGeometries   = &geometryDesc;
        ...
        // Create accleration structure object
        {
            ...
            createInfo.buffer                               = outBLASes[i].Buffer.Buffer;
            createInfo.offset                               = 0;
            createInfo.size                                 = buildSizesInfo.accelerationStructureSize;
            createInfo.type                                 = VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_KHR;

            CHECK_CALL(fn_vkCreateAccelerationStructureKHR(pRenderer->Device, &createInfo, nullptr, &outBLASes[i].AccelStruct));
        }
        ...
        //
        {
            ...
            buildGeometryInfo.flags                     = VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR;
            buildGeometryInfo.mode                      = VK_BUILD_ACCELERATION_STRUCTURE_MODE_BUILD_KHR;
            buildGeometryInfo.dstAccelerationStructure  = outBLASes[i].AccelStruct;
            buildGeometryInfo.geometryCount             = 1;
            buildGeometryInfo.pGeometries               = &geometryDesc;
            ...
            buildRangeInfos.primitiveCount                           = numTriangles;
            ...
            const VkAccelerationStructureBuildRangeInfoKHR* pBuildRangeInfo = &buildRangeInfos;
    ...
    VulkanAccelStruct*                    pTLAS)
{
    ...
    for (uint32_t i = 0; i < BLASes.size(); ++i)
    {
        ...
        instance.accelerationStructureReference     = GetDeviceAddress(pRenderer, BLASes[i].AccelStruct);
        memcpy(
            &instance.transform,
    ...
    const std::vector<Geometry>& geometries)
{
    ...
    VulkanBufferDescriptor materialsDescriptor;
    CreateDescriptor(
        pRenderer,
        &materialsDescriptor,
    ...
    std::vector<const VulkanBuffer*> triangleBuffers;
    std::vector<const VulkanBuffer*> positionBuffers;
    std::vector<const VulkanBuffer*> normalBuffers;

    for (uint32_t i = 0; i < geometries.size(); ++i)
    {
        triangleBuffers.push_back(&geometries[i].indexBuffer);
        positionBuffers.push_back(&geometries[i].positionBuffer);
        normalBuffers.push_back(&geometries[i].normalBuffer);
    }

    // StructuredBuffer<Triangle> Triangles[3] : register(t4); // Index buffer
    VulkanBufferDescriptor trianglesDescriptor(geometries.size());
    CreateDescriptor(
        pRenderer,
        &trianglesDescriptor,
        VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR,
    ...
    // StructuredBuffer<float3>   Positions[3] : register(t7); // Position buffer
    VulkanBufferDescriptor positionsDescriptor;
    CreateDescriptor(
        pRenderer,
        &positionsDescriptor,
        VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR,
    ...
    // StructuredBuffer<float3>   Normals[3]   : register(t10); // Normal buffer
    VulkanBufferDescriptor normalsDescriptor;
    CreateDescriptor(
        pRenderer,
        &normalsDescriptor,
        VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR,
    ...
    std::vector<VkDescriptorSetLayoutBinding> layoutBindings =
        {
            ...
            materialsDescriptor.layoutBinding,
            trianglesDescriptor.layoutBinding,
            positionsDescriptor.layoutBinding,
            normalsDescriptor.layoutBinding
        };
    ...
    std::vector<VkWriteDescriptorSet> writeDescriptorSets =
        {
            ...
            materialsDescriptor.writeDescriptorSet,
            trianglesDescriptor.writeDescriptorSet,
            positionsDescriptor.writeDescriptorSet,
            normalsDescriptor.writeDescriptorSet
        };
