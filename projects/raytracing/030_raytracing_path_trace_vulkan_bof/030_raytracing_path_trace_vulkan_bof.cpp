...
void CreateRayTracingDescriptors(
    ...
    VulkanImage*         pAccumTexture,
    VulkanBuffer*        pRayGenSamplesBuffer,
    VulkanBuffer*        pSceneParams,
    const Geometry&      sphereGeometry,
    const Geometry&      boxGeometry,
...
int main(int argc, char** argv)
{
    ...
    VulkanBuffer       readbackSwapchainSurface;
    VkBufferUsageFlags usageFlags                   = VK_BUFFER_USAGE_TRANSFER_DST_BIT;
    uint32_t           readbackSwapchainSurfaceSize = gWindowWidth * gWindowHeight * BytesPerPixel(GREX_DEFAULT_RTV_FORMAT);
    CHECK_CALL(CreateBuffer(
        renderer.get(),
        readbackSwapchainSurfaceSize,
        usageFlags,
    ...
    while (window->PollEvents())
    {
        ...
        // ImGui
        {
            if (gSelectedPixelChanged)
            {
                ...
                readbackRegion.imageExtent.width               = gWindowWidth;
                readbackRegion.imageExtent.height              = gWindowHeight;
                readbackRegion.imageExtent.depth               = 1;
                readbackRegion.imageOffset                     = {0, 0, 0};
                ...
                vkCmdCopyImageToBuffer(
                    ...
                    readbackSwapchainSurface.Buffer,
                    1,
                    &readbackRegion);
        ...
        if (gSelectedPixelChanged)
        {
            ...
            if (pReadBackData)
            {
                ...
                gSelectedColor[2]  = pReadBackData[pixelOffset + 0] / 255.0f;

                vmaUnmapMemory(renderer->Allocator, readbackSwapchainSurface.Allocation);
                pReadBackData = nullptr;
            }
    ...
    VkImageView*         pAccumImageView)
{
    ...
    VulkanBufferDescriptor geoIndexBufferDescriptor(kNumSpheres);
    VulkanBufferDescriptor geoPositionBufferDescriptor(kNumSpheres);
    VulkanBufferDescriptor geoNormalBufferDescriptor(kNumSpheres);
    ...
    VulkanImageDescriptor iblTextureDescriptor;
    {
        VkImageView imageView = VK_NULL_HANDLE;
        CHECK_CALL(CreateImageView(
            pRenderer,
