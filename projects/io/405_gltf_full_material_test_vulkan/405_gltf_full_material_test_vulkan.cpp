...
int main(int argc, char** argv)
{
    ...
    // *************************************************************************
    {
        ...
        // IBL Textures
        {
            ...
            {
                SceneData sceneData               = {};
                sceneData.IBLEnvironmentNumLevels = envNumLevels[0];

                VulkanBuffer sceneDataBuffer;
                CHECK_CALL(CreateBuffer(
                    renderer.get(),
                    Align<size_t>(sizeof(SceneData), 256),
                    &sceneData,
            ...
            for (size_t i = 0; i < envTextures.size(); ++i)
            {
                ...
                WriteDescriptor(
                    ...
                    pDescriptorBufferStartAddress,
                    pipelineLayout.DescriptorSetLayout,
                    IBL_ENV_MAP_TEXTURE_START_REGISTER,
            ...
            for (size_t i = 0; i < irrTextures.size(); ++i)
            {
                ...
                WriteDescriptor(
                    ...
                    pDescriptorBufferStartAddress,
                    pipelineLayout.DescriptorSetLayout,
                    IBL_IRR_MAP_TEXTURE_START_REGISTER,
                    static_cast<uint32_t>(i),
                    VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
                    imageView,
                    VK_IMAGE_LAYOUT_GENERAL);
            ...
            // IBL Map Sampler 
            {
                ...
                iblMapSampler.magFilter               = VK_FILTER_LINEAR;
                iblMapSampler.minFilter               = VK_FILTER_LINEAR;
                iblMapSampler.mipmapMode              = VK_SAMPLER_MIPMAP_MODE_LINEAR;
                iblMapSampler.addressModeU            = VK_SAMPLER_ADDRESS_MODE_REPEAT;
                iblMapSampler.addressModeV            = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
                iblMapSampler.addressModeW            = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
                iblMapSampler.mipLodBias              = 0;
                iblMapSampler.anisotropyEnable        = VK_FALSE;
                iblMapSampler.maxAnisotropy           = 0;
                iblMapSampler.compareEnable           = VK_FALSE;
                iblMapSampler.compareOp               = VK_COMPARE_OP_LESS_OR_EQUAL;
                iblMapSampler.minLod                  = 0;
                iblMapSampler.maxLod                  = FLT_MAX;
                iblMapSampler.borderColor             = VK_BORDER_COLOR_FLOAT_OPAQUE_BLACK;
                iblMapSampler.unnormalizedCoordinates = VK_FALSE;
                ...
                WriteDescriptor(
                    ...
                    pDescriptorBufferStartAddress,
                    pipelineLayout.DescriptorSetLayout,
                    IBL_MAP_SAMPLER_REGISTER, // binding
                    0,                        // arrayElement
                    repeatSampler);
            }
            ...
            // IBL Integration Sampler 
            {
                ...
                iblMapSampler.minFilter               = VK_FILTER_LINEAR;
                iblMapSampler.mipmapMode              = VK_SAMPLER_MIPMAP_MODE_LINEAR;
                iblMapSampler.addressModeU            = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
                iblMapSampler.addressModeV            = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
                iblMapSampler.addressModeW            = VK_SAMPLER_ADDRESS_MODE_CLAMP_TO_EDGE;
                iblMapSampler.mipLodBias              = 0;
                iblMapSampler.anisotropyEnable        = VK_FALSE;
                ...
                WriteDescriptor(
                    ...
                    pDescriptorBufferStartAddress,
                    pipelineLayout.DescriptorSetLayout,
                    IBL_INTEGRATION_SAMPLER_REGISTER, // binding
                    0,                                // arrayElement
                    repeatSampler);
            }
        }
        ...
        // Material Samplers
        {
           // Clamped
            {
                ...
                clampedSamplerInfo.compareEnable           = VK_FALSE;
           ...
           // Repeat
           {
                ...
                repeatSamplerInfo.compareEnable           = VK_FALSE;
    ...
    // Window
    // *************************************************************************
    auto window = GrexWindow::Create(gWindowWidth, gWindowHeight, "405_gltf_full_material_test_vulkan");
    if (!window)
    {
