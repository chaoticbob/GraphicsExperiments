...
struct TextureSet
{
    ...
    MetalTexture displacementTexture;
...
int main(int argc, char** argv)
{
    ...
    // Compile shaders
    // *************************************************************************
    MetalShader vsNormalMapShader;
    MetalShader fsNormalMapShader;
    {
        std::string shaderSource = LoadString("projects/308_normal_map_vs_pom/normal_shaders.metal");
        ...
        vsNormalMapShader.Function = NS::TransferPtr(library->newFunction(NS::String::string("vsmain", NS::UTF8StringEncoding)));
        if (vsNormalMapShader.Function.get() == nullptr)
        {
            assert(false && "VS Shader MTL::Library::newFunction() failed");
            return EXIT_FAILURE;
        }

        fsNormalMapShader.Function = NS::TransferPtr(library->newFunction(NS::String::string("psmain", NS::UTF8StringEncoding)));
        if (fsNormalMapShader.Function.get() == nullptr)
        {
    ...
    MetalShader vsPOMShader;
    MetalShader fsPOMShader;
    {
        std::string shaderSource = LoadString("projects/308_normal_map_vs_pom/parallax_shaders.metal");
        ...
        vsPOMShader.Function = NS::TransferPtr(library->newFunction(NS::String::string("vsmain", NS::UTF8StringEncoding)));
        if (vsPOMShader.Function.get() == nullptr)
        {
            assert(false && "VS Shader MTL::Library::newFunction() failed");
            return EXIT_FAILURE;
        }

        fsPOMShader.Function = NS::TransferPtr(library->newFunction(NS::String::string("psmain", NS::UTF8StringEncoding)));
        if (fsPOMShader.Function.get() == nullptr)
        {
            assert(false && "FS Shader MTL::Library::newFunction() failed");
            return EXIT_FAILURE;
        }
    ...
    // *************************************************************************
    MetalPipelineRenderState pomRenderPipelineState;
    MetalDepthStencilState   pomDepthStencilState;
    CHECK_CALL(CreateGraphicsPipeline1(
        ...
        &pomRenderPipelineState,
        &pomDepthStencilState));

    MetalPipelineRenderState normalMapRenderPipelineState;
    MetalDepthStencilState   normalMapDepthStencilState;
    CHECK_CALL(CreateGraphicsPipeline1(
        renderer.get(),
        &vsNormalMapShader,
        &fsNormalMapShader,
        GREX_DEFAULT_RTV_FORMAT,
        GREX_DEFAULT_DSV_FORMAT,
        &normalMapRenderPipelineState,
        &normalMapDepthStencilState));
    ...
    // Window
    // *************************************************************************
    auto window = GrexWindow::Create(gWindowWidth, gWindowHeight, "308_normal_map_vs_pom_metal");
    if (!window)
    {
    ...
    while (window->PollEvents())
    {
        ...
        mat4 projMat     = perspective(radians(60.0f), halfWindowWidth / static_cast<float>(gWindowHeight), 0.1f, 10000.0f);
        ...
        pRenderEncoder->setFragmentTexture(textureSet.displacementTexture.Texture.get(), 2);

        pRenderEncoder->setFragmentBytes(&heightMapScale, sizeof(float), 6);
        pRenderEncoder->setFragmentBytes(&enableDiscard, sizeof(bool), 7);
        pRenderEncoder->setFragmentBytes(&enableShadow, sizeof(bool), 8);
        pRenderEncoder->setFragmentBytes(&gShadowStep, sizeof(float), 9);
        ...
        // Draw normal map
        {
            pRenderEncoder->setRenderPipelineState(normalMapRenderPipelineState.State.get());
            pRenderEncoder->setDepthStencilState(normalMapDepthStencilState.State.get());

            MTL::Viewport viewport = {0, 0, static_cast<float>(halfWindowWidth), static_cast<float>(gWindowHeight), 0, 1};
        ...
        {
            pRenderEncoder->setRenderPipelineState(pomRenderPipelineState.State.get());
            pRenderEncoder->setDepthStencilState(pomDepthStencilState.State.get());

            MTL::Viewport viewport = {static_cast<float>(halfWindowWidth), 0, static_cast<float>(halfWindowWidth), static_cast<float>(gWindowHeight), 0, 1};
            pRenderEncoder->setViewport(viewport);

            MTL::ScissorRect scissor = {halfWindowWidth, 0, halfWindowWidth, gWindowHeight};
            pRenderEncoder->setScissorRect(scissor);
