...
void main()
{
   FragColor = vec4(texture(sampler2D(Tex0, Sampler0), TexCoord).xyz, 1);
}
)";
...
int main(int argc, char** argv)
{
    ...
    // Window
    // *************************************************************************
    auto window = Window::Create(gWindowWidth, gWindowHeight, "302_textured_cube_vulkan");
    if (!window) {
    ...
    while (window->PollEvents()) {
...

        {
            ...
            vkCmdPushConstants(cmdBuf.CommandBuffer, pipelineLayout.PipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(glm::mat4), &mvpMat);

            // Bind the Index Buffer
            vkCmdBindIndexBuffer(cmdBuf.CommandBuffer, indexBuffer.Buffer, 0, VK_INDEX_TYPE_UINT32);

            // Bind the Vertex Buffer
            VkBuffer     vertexBuffers[] = {positionBuffer.Buffer, texCoordBuffer.Buffer};
