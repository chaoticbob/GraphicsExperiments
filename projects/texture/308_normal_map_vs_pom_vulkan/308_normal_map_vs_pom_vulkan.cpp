...
struct TextureSet
{
    ...
    VulkanImage displacementTexture;
...
void CreateNormalMapPipelineLayout(VulkanRenderer* pRenderer, VulkanPipelineLayout* pLayout);
void CreatePomPipelineLayout(VulkanRenderer* pRenderer, VulkanPipelineLayout* pLayout);
...
int main(int argc, char** argv)
{
    ...
    // Compile shaders
    // *************************************************************************
    std::vector<uint32_t> normalMapSpirvVS;
    std::vector<uint32_t> normalMapSpirvFS;
    {
        std::string shaderSource = LoadString("projects/308_normal_map_vs_pom/normal_shaders.hlsl");

        std::string errorMsg;
        HRESULT     hr = CompileHLSL(shaderSource, "vsmain", "vs_6_0", &normalMapSpirvVS, &errorMsg);
        if (FAILED(hr)) {
            std::stringstream ss;
            ss << "\n"
        ...
        hr = CompileHLSL(shaderSource, "psmain", "ps_6_0", &normalMapSpirvFS, &errorMsg);
        if (FAILED(hr)) {
            std::stringstream ss;
            ss << "\n"
    ...
    std::vector<uint32_t> pomSpirvVS;
    std::vector<uint32_t> pomSpirvFS;
    {
        std::string shaderSource = LoadString("projects/308_normal_map_vs_pom/parallax_shaders.hlsl");

        std::string errorMsg;
        HRESULT     hr = CompileHLSL(shaderSource, "vsmain", "vs_6_0", &pomSpirvVS, &errorMsg);
        if (FAILED(hr)) {
            std::stringstream ss;
            ss << "\n"
               << "Shader compiler error (VS): " << errorMsg << "\n";
        ...
        hr = CompileHLSL(shaderSource, "psmain", "ps_6_0", &pomSpirvFS, &errorMsg);
        if (FAILED(hr)) {
            std::stringstream ss;
            ss << "\n"
               << "Shader compiler error (PS): " << errorMsg << "\n";
    ...
    // Shader module
    // *************************************************************************
    VkShaderModule normalMapModuleVS = VK_NULL_HANDLE;
    VkShaderModule normalMapModuleFS = VK_NULL_HANDLE;
    CreateShaderModules(
        renderer.get(),
        normalMapSpirvVS,
        normalMapSpirvFS,
    ...
    VkShaderModule pomModuleVS = VK_NULL_HANDLE;
    VkShaderModule pomModuleFS = VK_NULL_HANDLE;
    CreateShaderModules(
        renderer.get(),
        pomSpirvVS,
        pomSpirvFS,
    ...
    //
    // *************************************************************************
    VulkanPipelineLayout normalMapPipelineLayout = {};
    CreateNormalMapPipelineLayout(renderer.get(), &normalMapPipelineLayout);

    VulkanPipelineLayout pomPipelineLayout = {};
    CreatePomPipelineLayout(renderer.get(), &pomPipelineLayout);

    // *************************************************************************
    // Graphics pipeline state object
    // *************************************************************************
    VkPipeline pomPipelineState;
    CHECK_CALL(CreateGraphicsPipeline1(
        renderer.get(),
        pomPipelineLayout.PipelineLayout,
        pomModuleVS,
        pomModuleFS,
    ...
    VkPipeline normalMapPipelineState;
    CHECK_CALL(CreateGraphicsPipeline1(
        renderer.get(),
        normalMapPipelineLayout.PipelineLayout,
        normalMapModuleVS,
        normalMapModuleFS,
        GREX_DEFAULT_RTV_FORMAT,
        GREX_DEFAULT_DSV_FORMAT,
    ...
    CreateDescriptorBuffer(renderer.get(), normalMapPipelineLayout.DescriptorSetLayout, &envDescriptorBuffer);
    ...
    // Window
    // *************************************************************************
    auto window = Window::Create(gWindowWidth, gWindowHeight, "308_normal_map_vs_pom_vulkan");
    if (!window) {
    ...
    while (window->PollEvents()) {
        ...
        if (currentTextureSetIndex != textureSetIndex) {
            ...
            WriteDescriptors(
                renderer.get(),
                normalMapPipelineLayout.DescriptorSetLayout,
                &envDescriptorBuffer,
                textureSet,
                sampler);
        }
...

        {
          CmdTransitionImageLayout(
            ...
            // Draw normal map
            {
                vkCmdBindPipeline(cmdBuf.CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, normalMapPipelineState);

                VkViewport viewport = {0, static_cast<float>(gWindowHeight), static_cast<float>(halfWindowWidth), -static_cast<float>(gWindowHeight), 0.0f, 1.0f};
                ...
                vkCmdPushConstants(cmdBuf.CommandBuffer, normalMapPipelineLayout.PipelineLayout, VK_SHADER_STAGE_ALL_GRAPHICS, 0, sizeof(CameraProperties), &cameraParams);
            ...
            {
                vkCmdBindPipeline(cmdBuf.CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pomPipelineState);

                VkViewport viewport = {static_cast<float>(halfWindowWidth), static_cast<float>(gWindowHeight), static_cast<float>(halfWindowWidth), -static_cast<float>(gWindowHeight), 0.0f, 1.0f};
                vkCmdSetViewport(cmdBuf.CommandBuffer, 0, 1, &viewport);
                ...
                constantParams.HeightMapScale              = heightMapScale;
                constantParams.EnableDiscard               = enableDiscard;
                constantParams.EnableShadow                = enableShadow;
                constantParams.ShadowStep                  = gShadowStep;
                constantParams.Camera.ModelMatrix          = modelMat;
                constantParams.Camera.ViewProjectionMatrix = projViewMat;
                constantParams.Camera.EyePosition          = eyePos;

                vkCmdPushConstants(cmdBuf.CommandBuffer, pomPipelineLayout.PipelineLayout, VK_SHADER_STAGE_ALL_GRAPHICS, 0, sizeof(Constants), &constantParams);
...
void CreatePomPipelineLayout(VulkanRenderer* pRenderer, VulkanPipelineLayout* pLayout)
{
    ...
    VkPushConstantRange push_constant = {};
    push_constant.offset              = 0;
    push_constant.size                = sizeof(Constants);
    push_constant.stageFlags          = VK_SHADER_STAGE_ALL_GRAPHICS;
    ...
    VkSampler             sampler)
{
    ...
    // Texture2D                        DisplacementTexture : register(t3); // Texture
    {
        ...
        CHECK_CALL(CreateImageView(
            pRenderer,
            &textureSet.displacementTexture,
            VK_IMAGE_VIEW_TYPE_2D,
            VK_FORMAT_R8G8B8A8_UNORM,
            GREX_ALL_SUBRESOURCES,
            &imageView));
