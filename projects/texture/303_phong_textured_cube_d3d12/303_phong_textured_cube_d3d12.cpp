...
Texture2D                        Texture0 : register(t1); // Texture
...
VSOutput vsmain(float3 PositionOS : POSITION, float2 TexCoord : TEXCOORD, float3 Normal : NORMAL)
{
    ...
    output.Normal = mul(Camera.ModelMatrix, float4(Normal, 0)).xyz;
    return output;
}
...
int main(int argc, char** argv)
{
    ...
    // Window
    // *************************************************************************
    auto window = GrexWindow::Create(gWindowWidth, gWindowHeight, "303_phong_textured_cube_d3d12");
    if (!window)
    {
    ...
    while (window->PollEvents())
    {
        ...
        commandList->ResourceBarrier(1, &preRenderBarrier);
        {
            ...
            commandList->IASetVertexBuffers(0, 3, vbvs);
