...
int main(int argc, char** argv)
{
    ...
    // Window
    // *************************************************************************
    auto window = Window::Create(gWindowWidth, gWindowHeight, "308_normal_map_vs_pom_d3d12");
    if (!window) {
    ...
    const TextureSet&     textureSet)
{
    ...
    // Displacement
    srvDesc.Format              = textureSet.displacementTexture->GetDesc().Format;
    srvDesc.Texture2D.MipLevels = textureSet.displacementTexture->GetDesc().MipLevels;
    pRenderer->Device->CreateShaderResourceView(textureSet.displacementTexture.Get(), &srvDesc, descriptor);
    descriptor.ptr += inc;
}
