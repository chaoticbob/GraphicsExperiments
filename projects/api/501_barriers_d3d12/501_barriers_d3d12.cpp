...
int main(int argc, char** argv)
{
    ...
    if (SUCCEEDED(renderer->Device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS12, &options12, sizeof(options12))))
    {
    ...
    while (window->PollEvents())
    {
        ...
        D3D12_TEXTURE_BARRIER preRenderBarrier = CreateTextureBarrier(
            swapchainBuffer.Get(),
        ...
        D3D12_BARRIER_GROUP   preRenderBarrierGroup = {};

        preRenderBarrierGroup.NumBarriers = 1;
        preRenderBarrierGroup.Type        = D3D12_BARRIER_TYPE_TEXTURE;
        preRenderBarrierGroup.pTextureBarriers = &preRenderBarrier;

        commandList->Barrier(1, &preRenderBarrierGroup);
        ...
        D3D12_TEXTURE_BARRIER postRenderBarrier = CreateTextureBarrier(
            swapchainBuffer.Get(),
            D3D12_BARRIER_SYNC_DRAW,
            D3D12_BARRIER_SYNC_NONE,
            D3D12_BARRIER_ACCESS_RENDER_TARGET,
        ...
        D3D12_BARRIER_GROUP postRenderBarrierGroup = {};
        postRenderBarrierGroup.NumBarriers         = 1;
        postRenderBarrierGroup.Type                = D3D12_BARRIER_TYPE_TEXTURE;
        postRenderBarrierGroup.pTextureBarriers     = &postRenderBarrier;

        commandList->Barrier(1, &postRenderBarrierGroup);
