...
static void onSurfaceCreated(GLFMDisplay* pDisplay, int width, int height)
{
    ...
    {
        std::string shaderSource = LoadString("projects/111_mesh_shader_meshlets/shaders.msl");
...
static void onRender(GLFMDisplay* pDisplay)
{
    ...
    PerspCamera camera = PerspCamera(60.0f, static_cast<float>(gWindowWidth) / static_cast<float>(gWindowHeight));
    camera.LookAt(vec3(0, 0.105f, 0.40f), vec3(0, 0.105f, 0));

    mat4 R = glm::rotate(static_cast<float>(glfmGetTime()), glm::vec3(0, 1, 0));
    ...
    pRenderEncoder->drawMeshThreadgroups(MTL::Size(static_cast<uint32_t>(gApp->meshlets.size()), 1, 1), MTL::Size(0, 0, 0), MTL::Size(128, 1, 1));
