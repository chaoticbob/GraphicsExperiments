...
static int gVisibilityFunc = VISIBILITY_FUNC_CONE;
...
struct App {
    ...
    std::vector<float4x4> instances = std::vector<float4x4>(kNumInstanceCols * kNumInstanceRows);
...
static bool onTouch(GLFMDisplay* pDisplay, int touch, GLFMTouchPhase, double x, double y);
static void onSurfaceResized(GLFMDisplay* pDisplay, int width, int height);
...
void glfmMain(GLFMDisplay* pDisplay)
{
    ...
    glfmSetTouchFunc(pDisplay, onTouch);
    glfmSetSurfaceCreatedFunc(pDisplay, onSurfaceCreated);
    glfmSetSurfaceResizedFunc(pDisplay, onSurfaceResized);
...
static void onSurfaceCreated(GLFMDisplay* pDisplay, int width, int height)
{
    ...
    {
        std::string shaderSource = LoadString("projects/114_mesh_shader_culling/shaders.msl");
    ...
    // Render pipeline state
    {
        ...
        // Depth stencil state
        {
            ...
            if (desc.get() != nullptr) {
                ...
                gApp->depthStencilState.State = NS::TransferPtr(gApp->renderer->Device->newDepthStencilState(desc.get()));
                if (gApp->depthStencilState.State.get() == nullptr) {
    ...
    std::vector<uint8_t>         meshletTriangles;
    {
        ...
        gApp->meshBounds = mesh.GetBounds();
        ...
        gApp->meshlets.resize(maxMeshlets);
        ...
        auto& last = gApp->meshlets[meshletCount - 1];
        meshletVertices.resize(last.vertex_offset + last.vertex_count);
        meshletTriangles.resize(last.triangle_offset + ((last.triangle_count * 3 + 3) & ~3));
        gApp->meshlets.resize(meshletCount);
    }

    // Meshlet bounds (we're using bounding spheres)
    for (auto& meshlet : gApp->meshlets)
    {
        auto bounds = meshopt_computeMeshletBounds(
            ...
            positions.size(),
            sizeof(float3));
        gApp->meshletBounds.push_back(float4(bounds.center[0], bounds.center[1], bounds.center[2], bounds.radius));
    }

    // Get some counts to use later
    uint64_t meshletVertexCount   = 0;
    uint64_t meshletTriangleCount = 0;
    for (auto& m : gApp->meshlets)
    {
    ...
    // Create buffers
    {
        CHECK_CALL(CreateBuffer(gApp->renderer.get(), SizeInBytes(positions), DataPtr(positions), &gApp->positionBuffer));
        CHECK_CALL(CreateBuffer(gApp->renderer.get(), SizeInBytes(gApp->meshlets), DataPtr(gApp->meshlets), &gApp->meshletBuffer));
        CHECK_CALL(CreateBuffer(gApp->renderer.get(), SizeInBytes(meshletVertices), DataPtr(meshletVertices), &gApp->meshletVerticesBuffer));
        CHECK_CALL(CreateBuffer(gApp->renderer.get(), SizeInBytes(meshletTrianglesU32), DataPtr(meshletTrianglesU32), &gApp->meshletTrianglesBuffer));
        CHECK_CALL(CreateBuffer(gApp->renderer.get(), SizeInBytes(gApp->meshletBounds), DataPtr(gApp->meshletBounds), &gApp->meshletBoundsBuffer));
    }

    // *************************************************************************
    // Instances
    // *************************************************************************
    gApp->instances.resize(kNumInstanceCols * kNumInstanceRows);

    CreateBuffer(gApp->renderer.get(), SizeInBytes(gApp->instances), nullptr, &gApp->instancesBuffer);
}

static bool onTouch(GLFMDisplay* pDisplay, int touch, GLFMTouchPhase phase, double x, double y)
{
...
static void onRender(GLFMDisplay* pDisplay)
{
    ...
    {
        float maxSpan       = std::max<float>(gApp->meshBounds.Width(), gApp->meshBounds.Depth());
        ...
        for (uint32_t j = 0; j < kNumInstanceRows; ++j)
        {
            for (uint32_t i = 0; i < kNumInstanceCols; ++i)
            {
                ...
                float    t             = static_cast<float>(glfmGetTime()) + ((i ^ j + i) / 10.0f);
                gApp->instances[index] = glm::translate(float3(x, y, z)) * glm::rotate(t, float3(0, 1, 0));
            }
        }
    }
    ...
    // Update scene
    {
        ...
        gAngle += (gTargetAngle - gAngle) * 0.05f;
        mat4 rotMat = glm::rotate(glm::radians(gAngle), float3(0, 1, 0));
        target      = rotMat * float4(target, 1.0);

        PerspCamera camera = PerspCamera(45.0f, static_cast<float>(gWindowWidth) / static_cast<float>(gWindowHeight), 0.1f, farDist);
        ...
        gApp->scene.CameraVP                             = camera.GetViewProjectionMatrix();
        gApp->scene.Frustum.Planes[FRUSTUM_PLANE_LEFT]   = {frLeft.Normal, 0.0f, frLeft.Position, 0.0f};
        gApp->scene.Frustum.Planes[FRUSTUM_PLANE_RIGHT]  = {frRight.Normal, 0.0f, frRight.Position, 0.0f};
        gApp->scene.Frustum.Planes[FRUSTUM_PLANE_TOP]    = {frTop.Normal, 0.0f, frTop.Position, 0.0f};
        gApp->scene.Frustum.Planes[FRUSTUM_PLANE_BOTTOM] = {frBottom.Normal, 0.0f, frBottom.Position, 0.0f};
        gApp->scene.Frustum.Planes[FRUSTUM_PLANE_NEAR]   = {frNear.Normal, 0.0f, frNear.Position, 0.0f};
        gApp->scene.Frustum.Planes[FRUSTUM_PLANE_FAR]    = {frFar.Normal, 0.0f, frFar.Position, 0.0f};
        gApp->scene.Frustum.Sphere                       = camera.GetFrustumSphere();
        gApp->scene.Frustum.Cone.Tip                     = frCone.Tip;
        gApp->scene.Frustum.Cone.Height                  = frCone.Height;
        gApp->scene.Frustum.Cone.Direction               = frCone.Dir;
        gApp->scene.Frustum.Cone.Angle                   = frCone.Angle;
        gApp->scene.InstanceCount                        = static_cast<uint32_t>(gApp->instances.size());
        gApp->scene.MeshletCount                         = static_cast<uint32_t>(gApp->meshlets.size());
        gApp->scene.VisibilityFunc                       = gVisibilityFunc;
    }
    ...
    {
        memcpy(gApp->instancesBuffer.Buffer->contents(), DataPtr(gApp->instances), SizeInBytes(gApp->instances));
        //gApp->instancesBuffer.Buffer->didModifyRange(NS::Range::Make(0, SizeInBytes(gApp->instances)));
    }
    ...
    //
    pRenderEncoder->setObjectBytes(&gApp->scene, sizeof(SceneProperties), 0);
    pRenderEncoder->setObjectBuffer(gApp->meshletBoundsBuffer.Buffer.get(), 0, 1);
    pRenderEncoder->setObjectBuffer(gApp->instancesBuffer.Buffer.get(), 0, 2);
    //
    pRenderEncoder->setMeshBytes(&gApp->scene, sizeof(SceneProperties), 0);
    pRenderEncoder->setMeshBuffer(gApp->positionBuffer.Buffer.get(), 0, 1);
    pRenderEncoder->setMeshBuffer(gApp->meshletBuffer.Buffer.get(), 0, 2);
    pRenderEncoder->setMeshBuffer(gApp->meshletVerticesBuffer.Buffer.get(), 0, 3);
    pRenderEncoder->setMeshBuffer(gApp->meshletTrianglesBuffer.Buffer.get(), 0, 4);
    pRenderEncoder->setMeshBuffer(gApp->instancesBuffer.Buffer.get(), 0, 5);

    // Object function uses 32 for thread group size
    uint32_t threadGroupCountX = static_cast<uint32_t>((gApp->meshlets.size() / 32) + 1) * static_cast<uint32_t>(gApp->instances.size());
