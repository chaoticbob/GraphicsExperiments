...
int main(int argc, char** argv)
{
    ...
    {
        std::string shaderSource = LoadString("projects/119_mesh_shader_bary/shaders.metal");
    ...
    MetalBuffer meshletTrianglesBuffer;
    {
        ...
        CHECK_CALL(CreateBuffer(renderer.get(), SizeInBytes(texCoords), DataPtr(texCoords), &texCoordBuffer));
    ...
    while (window->PollEvents()) {
        ...
        pRenderEncoder->setMeshBuffer(texCoordBuffer.Buffer.get(), 0, 2);
        ...
        pRenderEncoder->setFragmentBuffer(positionBuffer.Buffer.get(), 0, 1);
        pRenderEncoder->setFragmentBuffer(texCoordBuffer.Buffer.get(), 0, 2);
        pRenderEncoder->setFragmentBuffer(normalsBuffer.Buffer.get(), 0, 3);
